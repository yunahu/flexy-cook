...
const FlexyCookProject = TodoistProjectsMock.find(
  (p) => p.name === "FlexyCook"
);
...
describe("Toolbar", () => {
  test("should render the 'convert to cooking steps' button", async () => {
    ...
    const button = result.container.querySelector(
      "#convert-to-cooking-steps-button"
    );
  ...
  test("'convert to cooking steps' button should add the cooking steps as tasks to a new section", async () => {
    const expected_tasks = GetRecipeMock.analyzedInstructions
      .map((instruction) =>
    ...
    const expected_sections = GetRecipeMock.analyzedInstructions.map(
      (instruction) => ({
    ...
    await waitFor(() => expect(alertMock).toHaveBeenCalled(1));
