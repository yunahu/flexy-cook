...
const TodoistShoppingListSection = TodoistSectionsMock.find(
  (section) => section.name === "Shopping List"
);
const addedTasks = [];
...
const server = setupServer(
  ...
  http.post(`${TODOIST_BASE_URL}/sections`, () =>
    HttpResponse.json(TodoistShoppingListSection)
  ),
...
describe("Toolbar", () => {
  ...
  test("'add to shopping list' button should add the ingredients as tasks in the shopping list section", async () => {
    const expected_tasks = GetRecipeMock.extendedIngredients.map(
      (ingredient) => ({
